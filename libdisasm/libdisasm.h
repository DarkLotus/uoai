...
__declspec(dllexport) int x86_init( enum x86_options options, DISASM_REPORTER reporter, void *arg);
...
__declspec(dllexport) int x86_cleanup(void);
 ...
 */
__declspec(dllexport) unsigned int x86_disasm( unsigned char *buf, unsigned int buf_len,
...
__declspec(dllexport) int x86_format_insn(x86_insn_t *insn, char *buf, int len, enum x86_asm_format);
