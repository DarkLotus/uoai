...
asm_function * disasm_macro_chunk(Stream ** clientstream, unsigned int macronumber, UOCallibrations * callibrations)
{
	...
	return disasm_chunk((psUInt)SReadUInt(clientstream));
}

int psUInt_compare(psUInt a, psUInt b)
{
	if(a>b)
		return -1;
	else if(a<b)
...
psUInt FindLoop(asm_function * onfunction)
{
	...
	seen_addresses=BT_create((BTCompare)psUInt_compare);
...
psUInt TracePushTarget(asm_function * curfunc, unsigned int registerid)//handling of mov reg, var; ...; push reg;
{
...
psUInt GetCallParameter(asm_function * curfunc, unsigned int parnumber)//finds parameter push, if op=reg it traces back the push
{
	insn_mask * push;
	psUInt toreturn;
	BinaryTreeNode * backup;
...
asm_function * SwitchToPacketHandler(unsigned char packetnumber, int followcall, int chunked, Stream ** clientstream, Process * clientprocess, UOCallibrations * callibrations)
{
	...
	{
		SSetPos(clientstream, ((psUInt)callibrations->PacketSwitch_indices)+packetnumber-0xB);
		targetaddr=(psUInt)SReadByte(clientstream);
		SSetPos(clientstream, ((psUInt)callibrations->PacketSwitch_offsets) + 4*targetaddr);
		targetaddr=(psUInt)SReadUInt(clientstream);
...
LinkedList * GetFieldOffsets(asm_function * curfunc, int search_sub_procedures, int minimaloffset)
{
	...
	offsets=BT_create((BTCompare)psUInt_compare);
...
int AOSPropertiesCallibrations(Stream ** clientstream, Process * clientprocess, UOCallibrations * callibrations)
{
	...
	*/
	...
	if((curinsn=find_insn(ShowItemPropGump, push_FFh, 0, 0))&&(curinsn=find_insn(ShowItemPropGump, push_FFh, 0, 0)))
	{
		...
		//while((curinsn=find_insn(ShowItemPropGump, call_relative, 0, 0))&&(((psUInt)callibrations->Allocator)!=insn_get_target(curinsn, 0)))
		//	;
...
int FindGumpConstructor(asm_function * onfunction, Stream ** clientstream, Process * clientprocess, UOCallibrations * callibrations, psUInt * constructor, psUInt * size)
{
	int toreturn;
	...
	if(((psUInt)callibrations->Allocator)==0)
		return 0;
	...
	while((curinsn=find_insn(onfunction, call_relative, 0, 0))&&(((psUInt)callibrations->Allocator)!=insn_get_target(curinsn, 0)))
		;
...
void AddGumpConstructor(BinaryTree * gumps_by_name, int needsubgumps, int checkall, psUInt pConstructor, psUInt Size, Stream ** clientstream, Process * clientprocess, UOCallibrations * callibrations)
{
	...
	psUInt sub_gump_constructor;
	psUInt sub_gump_size;
...
int GumpCallibrations(Stream ** clientstream, Process * clientprocess, UOCallibrations * callibrations)
{
	...
	psUInt gump_switch;
	psUInt curgumphandler;
	unsigned int i;
	psUInt cursize, curconstructor;
	...
	//-> OpenGump function
	if(open_gump_function=disasm_function((psUInt)callibrations->OpenGump))
	{
			...
			if(curchunk=disasm_function(curgump->pConstructor))
			{
				...
				if(curinsn)
				{
					...
					if(curinsn=find_insn(curchunk, cmp_reg_immediate, 0, 0))
					{
						...
						{
							toreturn++;//2
							callibrations->PartyMembersLength=(((unsigned int)(insn_get_target(curinsn, 1)-(psUInt)callibrations->PartyMembers))/4);
						}
...
int OtherPacketCallibrations(Stream ** clientstream, Process * clientprocess, UOCallibrations * callibrations)
{
	...
	psUInt SubCommandsSwitch_indices, SubCommandsSwitch_offsets;
	psUInt curaddr, LoginConfirmSwitch;
		...
		if(curinsn=find_insn(newitempackethandler, retn, 0, 0))
		{
			...
			if((curinsn=find_insn(newitempackethandler, call_relative, 0, 0))&&(targetaddr=insn_get_target(curinsn, 0)))
			{
				...
				{				
					...
					i=0;
					fields=(psUInt *)&(callibrations->ItemOffsets);

					while((curoffset=(psUInt)LL_dequeue(offsets))&&(i<19))
...
//	x86_op_datatype_t curdata;
	...
	if(curaddr=(psUInt)callibrations->NetworkObjectVtbl)
	{
		SSetPos(clientstream, curaddr+8*4);
		curaddr=(psUInt)SReadUInt(clientstream);
	}
	...
	if(curaddr)
	{
		if(vtbl_handle_packet=disasm_chunk(curaddr))
		{
			...
			{
				toreturn++;//1
				callibrations->pGameServerInfo=(psUInt)insn_get_target(curinsn, 0);
			}
...
int SendPacketCallibrations(Stream ** clientstream, Process * clientprocess, UOCallibrations * callibrations)
{
	...
	if(curfunc=disasm_function((psUInt)callibrations->SendPacket))
	{
...
int ToggleAlwaysRunMacroCallibrations(Stream ** clientstream, Process * clientprocess, UOCallibrations * callibrations)
{
	int toreturn;
	x86_insn_t * curinsn;
	asm_function * toggle_alwaysrun_macro_chunk;

	insn_mask * push, * call_relative, * call_relativeb, * mov_var_reg, * mov_var_regb;
	op_mask * curop;
...
int LastObjectMacroCallibrations(Stream ** clientstream, Process * clientprocess, UOCallibrations * callibrations)
{
	...
	psUInt loopaddress, findstatusgump_address;
	...
	if(lastobject_chunk=disasm_macro_chunk(clientstream, 16, callibrations))
	{
		...
		if(curinsn=find_insn(lastobject_chunk, call_relative, 0, 0))
		{
			...
			if(curfunc=disasm_function((psUInt)callibrations->FindItemByID))//callibrations within finditem_by_id
			{
				//mov eax, Player
		...
		if(curinsn=find_insn(lastobject_chunk, call_relative, 0, 0))
		{
			if(curfunc=disasm_function(insn_get_target(curinsn, 0)))//callibrations within LastObjectFunction
			{
				...
				{
					if(insn_get_target(curinsn, 0)==(psUInt)callibrations->LastObjectID)
						break;
				}
...
int MacroCallibrations(asm_function * winmain, UOCallibrations * callibrations)
{
	...
	if(curinsn=find_insn(winmain, push_0_call, 0, 0))
	{
		...
		if(curfunc=disasm_function(insn_get_target(curinsn, 0)))//follow call
		{
			...
			//find 4 calls in a row, then follow the 8th call found from there
			if(curinsn=find_insn(curfunc, _4_calls, 0, 1))
			{
				for(i=0;i<7;i++)
				...
				if(curinsn&&(curfuncb=disasm_function(insn_get_target(curinsn, 0))))
				{
					...
					if(curinsn=find_insn(curfuncb, call, 0, 0))
					{
						...
						if(curfuncc=disasm_chunk((psUInt)callibrations->Macro))
						{
...
int WinMainCallibrations(asm_function * winmain, UOCallibrations * callibrations)
{
	...
	if(curinsn=find_insn(winmain, xor_mov_mov, 0, 1))
	{
